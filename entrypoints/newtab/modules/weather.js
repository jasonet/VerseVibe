...
import { WMO_WEATHER_CODES } from "../config/index.js";
...
export class Weather {
  constructor() {
    ...
    this._autoDetectAttempted = false;
  ...
  autoDetectLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        state.set("yd_city", city);
        state.set("yd_lat", lat);
        state.set("yd_lon", lon);
        state.set("locationUpdate", Date.now());
      },
      (err) => {
        console.warn("[Weather] 自动获取浏览器位置失败（静默处理，天气保持空白）:", err);
        this.clearWeatherUI();
      },
  ...
  clearWeatherUI() {
    ...
    if (this.els.condition) this.els.condition.textContent = "";
    if (this.els.temp) this.els.temp.textContent = "";
    if (this.els.feelsLike) this.els.feelsLike.textContent = "";
    if (this.els.humidity) this.els.humidity.textContent = "";
    if (this.els.location) this.els.location.textContent = "";
    if (this.els.icon) this.els.icon.innerHTML = "";
    if (this.els.bar) this.els.bar.style.width = "0%";
    if (this.els.widget) {
      this.els.widget.style.opacity = "0";
      this.els.widget.style.pointerEvents = "none";
    }
  }
  ...
  async fetchData(onlyRender = false) {
    ...
    try {
      ...
      if (!coords) {
        // 默认显示浏览器当前位置：尝试自动获取 GPS 位置（静默）
        if (navigator.geolocation && !this._autoDetectAttempted) {
          this._autoDetectAttempted = true;
      ...
      if (this.els.widget) {
        this.els.widget.style.opacity = "";
        this.els.widget.style.pointerEvents = "";
      }
  ...
  getWmo(code) {
    ...
    if (isChinese && WMO_WEATHER_CODES && WMO_WEATHER_CODES[code]) {
      return {
        desc: WMO_WEATHER_CODES[code].label,
        icon: WMO_WEATHER_CODES[code].icon,
      };
    }
    const map = {
