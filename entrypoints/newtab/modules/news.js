...
async function hasNewsHostAccess(providers) {
  // VerseVibe 拥有全网 host_permissions，无需重复申请权限
  return true;
}
...
export class NewsManager {
  constructor() {
    ...
    window.addEventListener("resize", () => {
      if (this.getTotalCards() === 0 || this.getTotalCards() > 7) {
  ...
  getEffectiveTotalCards() {
    const configured = this.getTotalCards();
    if (configured > 0) return configured;
    // 0 = Auto（自适应全宽个数）：按当前容器宽度计算可舒适排满的卡片数
    const width = this.container?.clientWidth || window.innerWidth * 0.96;
    return Math.max(4, Math.min(25, Math.floor(width / 225)));
  }
  ...
  render() {
    ...
    const cache = state.get("newsCache") || CONFIG.defaults.newsCache;
    const selection = this.getSelection();
    const totalCards = this.getEffectiveTotalCards();
    this.container.dataset.totalCards = String(totalCards);
    this.container.style.setProperty("--news-columns", String(totalCards));
    const isScrollable = totalCards > 7;
    this.container.classList.toggle("news-scrollable", isScrollable);
    if (isScrollable) {
      const containerWidth = this.container.clientWidth || window.innerWidth * 0.96;
      const visibleCards = Math.max(4, Math.min(totalCards, Math.floor(containerWidth / 225)));
      this.container.style.setProperty("--visible-cards", String(visibleCards));
    } else {
      this.container.style.setProperty("--visible-cards", String(totalCards));
    }
