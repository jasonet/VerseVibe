...
import { secondStorage } from "../core/secondStorage.js";
import {
...
export class SettingsManager {
  ...
  setupEventListeners() {
    ...
    if (this.els.githubBtn) {
      this.els.githubBtn.addEventListener("click", () => {
        window.open(
          "https://github.com/jasonet/VerseVibe",
          "_blank",
        );
      });
    }
  ...
  async detectLocation() {
    ...
    if (!navigator.geolocation) {
      console.warn("[Location] 当前浏览器不支持地理定位");
      return;
    }
  ...
  _requestBrowserLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        if (requestId !== this._gpsRequestId) return;
        console.warn("[Location] 浏览器无法获取当前位置（静默处理，不跳提示）:", err);
      },
  ...
  _getRandomBackgroundIdentity(url) {
    ...
    try {
      ...
      const bingId = parsed.searchParams?.get("id");
      if (bingId) return `bing:${bingId}`;
  ...
  async _fetchRandomWallpaper(excludedUrls = [], { priority = "high" } = {}) {
    ...
    for (let attempt = 0; attempt < 5; attempt += 1) {
      ...
      try {
        const bingApiUrl = `https://cn.bing.com/HPImageArchive.aspx?format=js&idx=0&n=8&mkt=zh-CN`;
        ...
        if (!response.ok) {
          throw new Error(`Bing Wallpaper API returned ${response.status}.`);
        }
        const json = await response.json();
        const images = Array.isArray(json?.images) ? json.images : [];
        if (!images.length) throw new Error("No images in Bing response");

        const available = images.filter((img) => {
          const id = `bing:${new URL(img.url, "https://cn.bing.com").searchParams?.get("id") || img.url}`;
          return !excluded.has(id);
        });
        const selected = (available.length ? available : images)[
          Math.floor(Math.random() * (available.length || images.length))
        ];

        const relativeUrl = selected.url;
        const finalUrl = relativeUrl.startsWith("http")
          ? relativeUrl
          : `https://cn.bing.com${relativeUrl}`;
        ...
        if (!imgResponse.ok) throw new Error("Failed to fetch Bing image binary");
        const wallpaperBlob = await imgResponse.blob();
