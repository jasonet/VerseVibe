...
export class FullSettingsModal {
  ...
  build() {
    ...
    const tabDefs = [
      ...
      { id: "fs-tab-sync", label: "Cloud Sync" },
    ];
    this.els.tabBtns = [];
    ...
    const syncPane = this.buildSyncPane();

    [generalPane, appearancePane, newsPane, shortcutsPane, dataPane, syncPane].forEach(
      (pane, i) => {
  ...
  buildSyncPane() {
    ...
    // Enable toggle
    const syncToggle = this._toggle("fs-sync-enable-toggle");
    this.els.fsSyncEnableToggle = syncToggle.input;

    // Server input
    const serverInput = this._el("input", {
      type: "url",
      className: "settings-input fs-sync-server-input",
      placeholder: "https://api.hkez.com",
      value: window.__yddSyncClient?.getServerUrl() || "https://api.hkez.com",
    });
    this.els.fsSyncServerInput = serverInput;

    // Key input + buttons
    const keyInput = this._el("input", {
      type: "text",
      className: "settings-input fs-sync-key-input",
      placeholder: "Paste or generate your secret sync key",
      value: window.__yddSyncClient?.getSyncKey() || "",
    });
    const genKeyBtn = this._el("button", {
      type: "button",
      className: "settings-button",
      textContent: "Generate Key",
    });
    const copyKeyBtn = this._el("button", {
      type: "button",
      className: "settings-button",
      textContent: "Copy Key",
    });
    this.els.fsSyncKeyInput = keyInput;
    this.els.fsSyncGenKeyBtn = genKeyBtn;
    this.els.fsSyncCopyKeyBtn = copyKeyBtn;

    const keyWrapper = this._el("div", { className: "fs-sync-key-wrapper" }, [
      keyInput,
      genKeyBtn,
      copyKeyBtn,
    ]);
    ...
    const pushBtn = this._el("button", {
      type: "button",
      className: "settings-button",
      textContent: "Upload Now",
    });
    const pullBtn = this._el("button", {
      type: "button",
      className: "settings-button",
      textContent: "Download Now",
    });
    const statusLabel = this._el("span", {
      className: "fs-sync-status-text",
      textContent: window.__yddSyncClient?.isEnabled()
        ? `Revision: ${window.__yddSyncClient.getRevision()}`
        : "Sync is currently disabled",
    });
    this.els.fsSyncPushBtn = pushBtn;
    this.els.fsSyncPullBtn = pullBtn;
    this.els.fsSyncStatusLabel = statusLabel;

    const actionsWrapper = this._el("div", { className: "fs-sync-actions-wrapper" }, [
      pushBtn,
      pullBtn,
      statusLabel,
    ]);
    ...
    // Content packs
    const fetchPackBtn = this._el("button", {
      type: "button",
      className: "settings-button",
      textContent: "Check Server Content Packs",
    });
    const packStatus = this._el("span", {
      className: "fs-sync-pack-status",
      textContent: "Ready to check official updates",
    });
    this.els.fsSyncPackBtn = fetchPackBtn;
    this.els.fsSyncPackStatus = packStatus;

    const packWrapper = this._el("div", { className: "fs-sync-pack-wrapper" }, [
      fetchPackBtn,
      packStatus,
    ]);

    pane.appendChild(
      this._section("Multi-Browser Cloud Sync (End-to-End Encrypted)", [
        this._row(
          "Enable Cloud Sync",
          "Synchronize your shortcuts, tasks, and settings across Chrome, Firefox, and Safari.",
          syncToggle.wrapper,
        ),
        this._row(
          "Server Endpoint",
          "The intermediate sync server URL (defaults to api.hkez.com).",
          serverInput,
        ),
        this._row(
          "Secret Sync Key",
          "Shared key used to encrypt and isolate your configuration. Keep this private.",
          keyWrapper,
        ),
        this._row(
          "Manual Sync Actions",
          "Force an immediate upload or download of your configuration.",
          actionsWrapper,
        ),
      ]),
    );
    ...
    pane.appendChild(
      this._section("Server Content Packs & Extensions", [
        this._row(
          "Official Content Packs",
          "Download curated AI tools, search engines, and presets from api.hkez.com.",
          packWrapper,
        ),
      ]),
    );
  ...
  bindCoreEvents() {
    ...
    this.els.fsUpdateBtn.addEventListener("click", () => {
      window.open(
        "https://github.com/jasonet/VerseVibe/releases",
        "_blank",
      );
    });
    this.els.fsGhBtn.addEventListener("click", () => {
      window.open("https://github.com/jasonet/VerseVibe", "_blank");
    });

    // Cloud Sync event bindings
    if (this.els.fsSyncEnableToggle) {
      this.els.fsSyncEnableToggle.checked = window.__yddSyncClient?.isEnabled() || false;
      this.els.fsSyncEnableToggle.addEventListener("change", () => {
        const client = window.__yddSyncClient;
        if (!client) return;
        if (this.els.fsSyncEnableToggle.checked) {
          const sUrl = this.els.fsSyncServerInput?.value?.trim() || "https://api.hkez.com";
          const sKey = this.els.fsSyncKeyInput?.value?.trim();
          if (!sKey) {
            import("../modules/syncCrypto.js").then(({ generateSyncKey }) => {
              const newKey = generateSyncKey();
              if (this.els.fsSyncKeyInput) this.els.fsSyncKeyInput.value = newKey;
              client.enable(sUrl, newKey);
              if (this.els.fsSyncStatusLabel) {
                this.els.fsSyncStatusLabel.textContent = "Sync enabled (new key generated)";
              }
            });
          } else {
            client.enable(sUrl, sKey);
            if (this.els.fsSyncStatusLabel) {
              this.els.fsSyncStatusLabel.textContent = "Sync enabled";
            }
          }
        } else {
          client.disable();
          if (this.els.fsSyncStatusLabel) {
            this.els.fsSyncStatusLabel.textContent = "Sync disabled";
          }
        }
      });
    }

    this.els.fsSyncGenKeyBtn?.addEventListener("click", async () => {
      const { generateSyncKey } = await import("../modules/syncCrypto.js");
      const newKey = generateSyncKey();
      if (this.els.fsSyncKeyInput) this.els.fsSyncKeyInput.value = newKey;
      if (this.els.fsSyncEnableToggle?.checked) {
        window.__yddSyncClient?.enable(
          this.els.fsSyncServerInput?.value?.trim() || "https://api.hkez.com",
          newKey,
        );
      }
    });

    this.els.fsSyncCopyKeyBtn?.addEventListener("click", () => {
      const val = this.els.fsSyncKeyInput?.value;
      if (val) {
        navigator.clipboard?.writeText(val);
        if (this.els.fsSyncCopyKeyBtn) {
          this.els.fsSyncCopyKeyBtn.textContent = "Copied!";
          setTimeout(() => {
            if (this.els.fsSyncCopyKeyBtn) this.els.fsSyncCopyKeyBtn.textContent = "Copy Key";
          }, 2000);
        }
      }
    });

    this.els.fsSyncPushBtn?.addEventListener("click", async () => {
      if (this.els.fsSyncStatusLabel) this.els.fsSyncStatusLabel.textContent = "Uploading...";
      try {
        await window.__yddSyncClient?.pushCurrentState();
        if (this.els.fsSyncStatusLabel) {
          this.els.fsSyncStatusLabel.textContent = `Uploaded! Rev: ${window.__yddSyncClient?.getRevision()}`;
        }
      } catch (err) {
        if (this.els.fsSyncStatusLabel) {
          this.els.fsSyncStatusLabel.textContent = `Upload failed: ${err.message}`;
        }
      }
    });

    this.els.fsSyncPullBtn?.addEventListener("click", async () => {
      if (this.els.fsSyncStatusLabel) this.els.fsSyncStatusLabel.textContent = "Downloading...";
      try {
        await window.__yddSyncClient?.pullAndApply();
        if (this.els.fsSyncStatusLabel) {
          this.els.fsSyncStatusLabel.textContent = `Downloaded! Rev: ${window.__yddSyncClient?.getRevision()}`;
        }
      } catch (err) {
        if (this.els.fsSyncStatusLabel) {
          this.els.fsSyncStatusLabel.textContent = `Download failed: ${err.message}`;
        }
      }
    });

    this.els.fsSyncPackBtn?.addEventListener("click", async () => {
      if (this.els.fsSyncPackStatus) this.els.fsSyncPackStatus.textContent = "Fetching packs...";
      try {
        const aiPack = await window.__yddSyncClient?.fetchContentPack("aitools");
        if (aiPack?.items) {
          if (this.els.fsSyncPackStatus) {
            this.els.fsSyncPackStatus.textContent = `Fetched aitools (${aiPack.items.length} items)!`;
          }
        } else {
          if (this.els.fsSyncPackStatus) {
            this.els.fsSyncPackStatus.textContent = "No new content found or server unreachable";
          }
        }
      } catch (err) {
        if (this.els.fsSyncPackStatus) {
          this.els.fsSyncPackStatus.textContent = `Pack check failed: ${err.message}`;
        }
      }
    });
  }
  ...
  buildNewsPane() {
    ...
    const totalCards = this._dropdown(
      "fs-news-total-cards",
      NEWS_CARD_COUNTS.map((count) => {
        if (count === 0) return [String(count), "Auto (自适应全宽)"];
        if (count === 5) return [String(count), "5 cards (默认)"];
        if (count === 25) return [String(count), "25 cards (全部排满)"];
        return [String(count), `${count} cards`];
      }),
    );
