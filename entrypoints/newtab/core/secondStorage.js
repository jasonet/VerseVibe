...
const DB_NAME = "VerseVibe_Dashboard_Storage";
...
export const secondStorage = {
  ...
  appendRandomBackgroundQueue(entries, limit = 2) {
    return enqueueMutation(() =>
      mutateRandomBackgroundQueue((queue) => {
        ...
        return {
          result: {
            appended: limitedQueue.length - queue.length,
            queue: limitedQueue,
          },
          queue: limitedQueue,
        };
  ...
  saveCurrentRandomBackground(entry) {
    return enqueueMutation(() =>
      runTransaction(
        "readwrite",
  ...
  async getCurrentRandomBackground() {
    await mutationQueue;
